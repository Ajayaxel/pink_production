  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const indoOutfitsProducts = allProducts.filter(p => p.category?.toLowerCase() === 'indo-western outfits');
        setProducts(indoOutfitsProducts);
        setFilteredProducts(indoOutfitsProducts);
      } catch (err) {
          ...
          {filteredProducts.length === 0 ? (
            <p className="text-center col-span-full text-gray-600">
              {hasActiveFilters() 
                ...
                : "🚫 No Indo outfits products found."
              }
            </p>
          ) : (
            filteredProducts.map(item => {
              ...
              return (
                <div key={item._id} className="bg-white rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 cursor-pointer group relative">
                  ...
                  {/* Image Container */}
                  <div onClick={() => navigate(`/shop-details/${item._id}`)}>
