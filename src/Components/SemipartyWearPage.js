  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const semipartywearProducts = allProducts.filter(p => p.category?.toLowerCase() === 'semi party wear');
        setProducts(semipartywearProducts);
        setFilteredProducts(semipartywearProducts);
      } catch (err) {
          ...
          {filteredProducts.length === 0 ? (
            <p className="text-center col-span-full text-gray-600">
              {hasActiveFilters() 
                ...
                : "🚫 No semi party wear products found."
              }
            </p>
          ) : (
            filteredProducts.map(item => {
              ...
              return (
                <div onClick={() => navigate(`/shop-details/${item._id}`)} key={item._id} className="bg-white rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 cursor-pointer group relative">
                  {/* New Badge */}
                  {Math.random() > 0.8 && (
